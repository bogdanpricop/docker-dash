  ...
  'observability': {
    en: {
      ...
      sections: [
        ...
        { icon: 'fa-key', title: 'Persistent collectors', body: 'An administrator can create an API key with only monitoring.read through the API Keys endpoint. It permits only metrics and detailed cluster-status GET/HEAD, while other reads and writes are refused. Use authorization type ApiKey and a private credential file in Prometheus. Without expiresAt it has no automatic expiry: replace and revoke it explicitly. Reissue collector keys when upgrading through the legacy-key revocation migration.' },
        { icon: 'fa-shield-halved', title: 'Grafana bootstrap password', body: 'The bundled profile also requires GRAFANA_ADMIN_PASSWORD_FILE. Generate a unique 20-256 character password in a private host directory; mount the file read-only and readable by Grafana. Missing, empty or invalid bootstrap files stop startup. The former admin/admin fallback is removed. Changing the file does not reset an existing account: change legacy passwords through Grafana or its administrator CLI.' },
    ...
    ro: {
      ...
      sections: [
        ...
        { icon: 'fa-key', title: 'Colectori permanenti', body: 'Un administrator poate crea prin endpoint-ul API Keys o cheie cu doar monitoring.read. Permite numai GET/HEAD pentru metrici si starea detaliata a clusterului; celelalte citiri si modificarile sunt refuzate. Folositi tipul de autorizare ApiKey si un fisier privat pentru Prometheus. Fara expiresAt nu expira automat: inlocuiti si revocati explicit cheia. Reemiteti cheile colectorilor la actualizarea care revoca vechile chei API.' },
        { icon: 'fa-shield-halved', title: 'Parola initiala Grafana', body: 'Profilul inclus necesita si GRAFANA_ADMIN_PASSWORD_FILE. Generati o parola unica de 20-256 de caractere intr-un director privat; montati fisierul doar pentru citire, accesibil utilizatorului Grafana. Un fisier lipsa, gol sau o parola prea scurta opreste pornirea. Parola implicita admin/admin a fost eliminata. Schimbarea fisierului nu reseteaza un cont existent: schimbati parolele vechi prin Grafana sau CLI-ul de administrare.' },
  ...
  'governance-controls': {
    en: {
      ...
      sections: [
        ...
        { icon: 'fa-building-shield', title: 'Tenant service credentials', body: 'A tenant token can read its own project, descendant scopes, capacity and filtered approval records. Applicable global or inherited policies and blackouts remain visible. governance.write or api.write permits capacity accounting changes with atomic audit. Foreign project selectors, membership/approval decisions and other authenticated routes are refused. This does not grant provider execution or user identity. Suspending a tenant permanently revokes its tokens; after reactivation, issue new credentials. Global monitoring requires an administrator or a global monitoring.read credential; tenant tokens cannot scrape it.' },
        { icon: 'fa-user-lock', title: 'Credential administration', body: 'Sign in as an administrator to manage realms, trusts and service tokens; API keys and service credentials cannot administer them. Issue, rotate and trust/realm changes commit with audit. If audit fails, the change is rolled back, including a trust disable/delete: repair audit and retry. Explicit token revocation also revokes descendants and remains effective if audit fails. Manual tokens may be renewed independently; workload rotations remain bounded by the original proof.' },
        { icon: 'fa-key', title: 'Workload exchange', body: 'Pinned public keys verify signed JWT proofs. Each signed proof and issuer/JWT-id pair can be redeemed once during its validity, including across trust recreation. Tokens never outlive the proof. Read-only mode refuses issuance; token, replay history and audit commit together. Upgrade 183 revokes prior workload tokens and their rotations. If unexpired old replay history exists, request a new proof after the upgrade cutoff (up to 61 seconds). Manual tokens are preserved. Migration 184 links new tokens and rotations to their trust: authority changes or deletion revoke them, and rotations cannot broaden scopes or outlive the proof. Legacy workload tokens are revoked because their trust cannot be reconstructed.' },
        ...
        { icon: 'fa-users-gear', title: 'SCIM provisioning', body: 'SCIM requires a global service token with the explicit scim.read or scim.write scope. User sessions, personal API keys and generic API scopes are refused. It lists and changes only SCIM-managed users and groups; local accounts and teams cannot be adopted. Group members must also be SCIM-managed. Tenant-scoped tokens are refused because this endpoint does not implement tenant isolation. Changes and audit entries commit together; an audit failure rolls the operation back. Automatic linking to OIDC accounts is not supported.' },
    ...
    ro: {
      ...
      sections: [
        ...
        { icon: 'fa-building-shield', title: 'Credentiale de serviciu pentru tenant', body: 'Un token de tenant poate citi proiectul propriu, scope-urile descendente, capacitatea si aprobarile filtrate. Politicile si perioadele de inghet globale sau mostenite care se aplica raman vizibile. governance.write sau api.write permite modificarea evidentei capacitatii cu audit atomic. Selectorii altui proiect, administrarea membrilor, deciziile de aprobare si celelalte rute autentificate sunt refuzate. Tokenul nu primeste executie pe provideri sau identitate de utilizator. Suspendarea tenantului revoca definitiv tokenurile; dupa reactivare trebuie emise credentiale noi. Monitorizarea globala necesita un administrator sau un credential global monitoring.read; tokenurile de tenant nu pot colecta aceste date.' },
        { icon: 'fa-user-lock', title: 'Administrarea credentialelor', body: 'Autentificati-va ca administrator pentru realm-uri, trusturi si tokenuri de serviciu; cheile API si credentialele de serviciu nu le pot administra. Emiterea, rotatia si modificarile trusturilor/realm-urilor se salveaza impreuna cu auditul. Daca auditul esueaza, schimbarea este anulata, inclusiv dezactivarea/stergerea unui trust: reparati auditul si reincercati. Revocarea explicita a tokenului revoca si descendentii si ramane efectiva daca auditul esueaza. Tokenurile manuale pot fi reinnoite independent; rotatiile workload raman limitate de dovada originala.' },
        { icon: 'fa-key', title: 'Schimb de dovezi workload', body: 'Cheile publice fixate verifica dovezile JWT semnate. Fiecare dovada semnata si pereche emitent/identificator JWT poate fi folosita o data pe durata validitatii, inclusiv dupa recrearea trustului. Tokenul nu depaseste expirarea dovezii. Modul read-only refuza emiterea; tokenul, istoricul replay si auditul se salveaza impreuna. Upgrade-ul 183 revoca tokenurile workload anterioare si rotatiile lor. Daca exista istoric replay vechi neexpirat, cereti o dovada noua dupa limita upgrade-ului (pana la 61 de secunde). Tokenurile manuale sunt pastrate. Migrarea 184 leaga tokenurile noi si rotatiile de trust: schimbarea autoritatii sau stergerea le revoca, iar rotatiile nu pot extinde scope-urile sau durata dovezii. Tokenurile workload vechi sunt revocate deoarece trustul lor nu poate fi reconstruit.' },
        ...
        { icon: 'fa-users-gear', title: 'Provisionare SCIM', body: 'SCIM necesita token de serviciu global cu scope explicit scim.read sau scim.write. Sesiunile utilizatorilor, cheile API personale si scope-urile API generice sunt refuzate. Listeaza si modifica numai utilizatori si grupuri administrate prin SCIM; nu poate prelua conturi sau echipe locale. Membrii grupurilor trebuie sa fie tot administrati prin SCIM. Tokenurile limitate la un tenant sunt refuzate deoarece endpointul nu implementeaza izolarea intre tenanti. Modificarea si auditul se salveaza impreuna; esecul auditului anuleaza operatia. Asocierea automata cu identitati OIDC nu este suportata.' },
