...
const NetworkPosturePage = {
  ...
  _conflictsHtml(result) { return `<div class="card" style="padding:16px;margin:16px 0"><strong>IP conflict candidates</strong><div class="text-muted text-sm">Repeated provider-visible addresses only; this does not confirm an IP conflict.</div><div class="stats-grid" style="margin-top:12px"><div class="stat-card"><div class="stat-value">${result.summary?.candidateCount ?? 0}</div><div class="stat-label">Candidates</div></div><div class="stat-card"><div class="stat-value">${result.summary?.observedAddressCount ?? 0}</div><div class="stat-label">Observed addresses</div></div></div></div>`; },
  _readinessHtml(result) { const s = result.summary || {}; return `<div class="card" style="padding:16px;margin:16px 0"><strong>Guest network readiness evidence</strong><div class="text-muted text-sm">NIC and address evidence only; no guest or network probe is performed.</div><div class="stats-grid" style="margin-top:12px"><div class="stat-card"><div class="stat-value">${s.readyCount ?? 0}</div><div class="stat-label">Ready evidence</div></div><div class="stat-card"><div class="stat-value">${s.notReadyCount ?? 0}</div><div class="stat-label">Not ready</div></div><div class="stat-card"><div class="stat-value">${s.unknownCount ?? 0}</div><div class="stat-label">Unknown</div></div></div></div>`; },
  _transportHtml(result) { return `<div class="card" style="padding:16px;margin:16px 0"><strong>Provider endpoint transport posture</strong><div class="text-muted text-sm">Existing capability-probe evidence only; this is not a new TLS, port or certificate scan.</div><span class="badge badge-secondary" style="margin-top:10px">${Utils.escapeHtml(result.state || 'unknown')}</span></div>`; },

  _evidenceDashboardHtml(results) {
    const value = (result, path) => {
      if (!result || result.error) return '—';
      return path.split('.').reduce((current, key) => current?.[key], result) ?? '—';
    };
    const unavailable = Object.values(results).filter(result => result?.error).length;
    return `<div class="card" style="padding:16px;margin-bottom:16px;border-left:4px solid var(--accent)"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div><strong><i class="fas fa-clipboard-check" aria-hidden="true"></i> Consolidated network evidence</strong><div class="text-muted text-sm">A read-only summary of the evidence collected below. It does not prove connectivity, isolation, routing, policy enforcement, or address ownership.</div></div><span class="badge ${unavailable ? 'badge-warning' : 'badge-secondary'}">${unavailable ? `${unavailable} source${unavailable === 1 ? '' : 's'} unavailable` : 'all sources returned'}</span></div><div class="stats-grid" style="margin-top:14px"><div class="stat-card"><div class="stat-value">${Utils.escapeHtml(value(results.posture, 'summary.networkCount'))}</div><div class="stat-label">Networks observed</div></div><div class="stat-card"><div class="stat-value">${Utils.escapeHtml(value(results.topology, 'summary.attachmentCount'))}</div><div class="stat-label">VM attachments</div></div><div class="stat-card"><div class="stat-value">${Utils.escapeHtml(value(results.ips, 'summary.addressCount'))}</div><div class="stat-label">IP observations</div></div><div class="stat-card"><div class="stat-value">${Utils.escapeHtml(value(results.conflicts, 'summary.candidateCount'))}</div><div class="stat-label">Conflict candidates</div></div><div class="stat-card"><div class="stat-value">${Utils.escapeHtml(value(results.readiness, 'summary.readyCount'))}</div><div class="stat-label">Ready evidence</div></div><div class="stat-card"><div class="stat-value">${Utils.escapeHtml(value(results.transport, 'state'))}</div><div class="stat-label">Endpoint transport</div></div></div></div>`;
  },
  ...
  async _load() {
    ...
    try { const [posture, policy, topology, placement, drift, ips, conflicts, readiness, transport] = await Promise.all([Api.getProviderNetworkPosture(this._hostId), Api.getProviderNetworkPolicyAdvisory(this._hostId, { minMtu: this._policyMinMtu, requireManaged: this._policyRequireManaged, requireVlan: this._policyRequireVlan }).catch(error => ({ error })), Api.getProviderNetworkAttachmentTopology(this._hostId).catch(error => ({ error })), Api.getProviderNetworkPlacementAdvisory(this._hostId).catch(error => ({ error })), Api.getProviderNetworkDriftBaseline(this._hostId).catch(error => ({ error })), Api.getProviderIpAddressInventory(this._hostId).catch(error => ({ error })), Api.getProviderIpConflictCandidates(this._hostId).catch(error => ({ error })), Api.getProviderGuestNetworkReadiness(this._hostId).catch(error => ({ error })), Api.getProviderEndpointTransportPosture(this._hostId).catch(error => ({ error }))]); target.innerHTML = this._evidenceDashboardHtml({ posture, policy, topology, placement, drift, ips, conflicts, readiness, transport }) + this._resultHtml(posture) + (policy.error ? `<div class="alert alert-info"><strong>Network policy advisory unavailable</strong><div>${Utils.escapeHtml(policy.error.message)}</div></div>` : this._policyHtml(policy)) + (topology.error ? `<div class="alert alert-info"><strong>Network attachment topology unavailable</strong><div>${Utils.escapeHtml(topology.error.message)}</div></div>` : this._topologyHtml(topology)) + (placement.error ? `<div class="alert alert-info"><strong>Network placement advisory unavailable</strong><div>${Utils.escapeHtml(placement.error.message)}</div></div>` : this._placementHtml(placement)) + (drift.error ? `<div class="alert alert-info"><strong>Network baseline unavailable</strong><div>${Utils.escapeHtml(drift.error.message)}</div></div>` : this._driftHtml(drift)) + (ips.error ? `<div class="alert alert-info"><strong>IP address inventory unavailable</strong><div>${Utils.escapeHtml(ips.error.message)}</div></div>` : this._ipHtml(ips)) + (conflicts.error ? `<div class="alert alert-info"><strong>IP conflict candidates unavailable</strong><div>${Utils.escapeHtml(conflicts.error.message)}</div></div>` : this._conflictsHtml(conflicts)) + (readiness.error ? `<div class="alert alert-info"><strong>Guest network readiness unavailable</strong><div>${Utils.escapeHtml(readiness.error.message)}</div></div>` : this._readinessHtml(readiness)) + (transport.error ? `<div class="alert alert-info"><strong>Endpoint transport posture unavailable</strong><div>${Utils.escapeHtml(transport.error.message)}</div></div>` : this._transportHtml(transport)); }
