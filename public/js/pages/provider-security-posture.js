...
const ProviderSecurityPosturePage = {
  ...
  async _load() {
    const target = this._container?.querySelector('#provider-security-content');
    if (!target) return;
    if (!this._hostId) {
      target.innerHTML = '<div class="empty-msg"><i class="fas fa-server"></i>Add a supported virtualization endpoint to inspect its declared safeguards.</div>';
      return;
    }
    target.innerHTML = '<div class="empty-msg"><i class="fas fa-spinner fa-spin"></i>Collecting declared capability evidence…</div>';
    try {
      const [result, assurance, securityLifecycle, privilegedCompliance, foundationQualification,
        networkBackupQualification, recoveryDepthQualification, drSecurityQualification,
        securityLifecycleQualification, privilegedClosureQualification] = await Promise.all([
        Api.getProviderSecurityPosture(this._hostId),
        Api.getProviderSecurityAssurance(this._hostId).catch(() => null),
        Api.getProviderSecurityLifecycle(this._hostId).catch(() => null),
        Api.getProviderPrivilegedCompliance(this._hostId).catch(() => null),
        Api.getProviderOperationalQualification(this._hostId).catch(() => null),
        Api.getProviderOperationalQualification(this._hostId, 'network-backup').catch(() => null),
        Api.getProviderOperationalQualification(this._hostId, 'recovery-depth').catch(() => null),
        Api.getProviderOperationalQualification(this._hostId, 'dr-security').catch(() => null),
        Api.getProviderOperationalQualification(this._hostId, 'security-lifecycle').catch(() => null),
        Api.getProviderOperationalQualification(this._hostId, 'privileged-closure').catch(() => null),
      ]);
      this._assurance = assurance; this._securityLifecycle = securityLifecycle;
      this._privilegedCompliance = privilegedCompliance;
      this._operationalQualifications = [foundationQualification, networkBackupQualification,
        recoveryDepthQualification, drSecurityQualification, securityLifecycleQualification,
        privilegedClosureQualification].filter(Boolean);
      const qualificationHtml = this._operationalQualifications.length
        ? this._operationalQualifications.map(item => this._operationalQualificationHtml(item)).join('')
        : this._operationalQualificationHtml(null);
      target.innerHTML = this._dashboardHtml(result) + qualificationHtml + this._assuranceHtml(assurance)
        + this._securityLifecycleHtml(securityLifecycle) + this._privilegedComplianceHtml(privilegedCompliance)
        + this._freshnessHtml(result) + this._coverageHtml(result) + this._safeguardsHtml(result)
        + this._recoveryHtml(result) + this._consoleHtml(result) + this._tasksHtml(result)
        + this._networkHtml(result) + this._lifecycleHtml(result) + this._gapsHtml(result)
        + `<div class="alert alert-info"><strong>Assessment limits</strong><ul>${(result.limitations || [])
          .map(item => `<li>${Utils.escapeHtml(item)}</li>`).join('')}</ul></div>`;
      this._wireAssurance();
    } catch (err) {
