...
function record({ inspect, hostId = 0, action, username = 'system' }) {
  ...
  const snapshot = sealSnapshot(JSON.stringify({ Image: cfg.Image, Cmd: cfg.Cmd, Env: cfg.Env,
    ExposedPorts: cfg.ExposedPorts, Labels: cfg.Labels, WorkingDir: cfg.WorkingDir,
    Entrypoint: cfg.Entrypoint, Volumes: cfg.Volumes, Hostname: cfg.Hostname,
    User: cfg.User, HostConfig: inspect.HostConfig }), entry);
  ...
  return getDb().prepare(`INSERT INTO container_image_history
    ...
    entry.host_id, cfg.Image, entry.image_id, action, username, inspect.State.Running ? 1 : 0, snapshot).lastInsertRowid;
}
