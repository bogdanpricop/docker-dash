...
<p>Discovery must return the exact configured issuer and valid HTTPS authorization, token and JWKS endpoints. Use a tenant-specific Entra issuer, not metadata with an unresolved tenant placeholder. Responses must be HTTP 200 JSON objects, at most 1 MiB, with a 10-second total deadline per upstream request. Redirects and compressed responses are refused. Concurrent discovery/key loads share a request; at most eight upstream requests run per process. Private CAs can be supplied through the Node trust store, including <code>NODE_EXTRA_CA_CERTS</code> at process startup; certificate verification must remain enabled.</p>
<p lang="ro">Discovery trebuie sa declare exact issuer-ul configurat si endpointuri HTTPS valide. Foloseste issuer-ul Entra pentru tenantul concret. Fiecare raspuns necesita HTTP 200 si obiect JSON, cu limita de 1 MiB si 10 secunde in total. Redirecturile si compresia sunt refuzate. Sunt permise opt cereri simultane per proces, iar descarcarile concurente de configuratie/chei sunt reunite. Pentru CA private, configureaza trust store-ul Node la pornire; nu dezactiva verificarea certificatului.</p>
<ul>
