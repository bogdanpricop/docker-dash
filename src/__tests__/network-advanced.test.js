...
describe('advanced network control plane', () => {
  ...
  test('routes expose eleven audited plan/evidence writes and no execution endpoint', () => {
    ...
    expect((route.match(/create\('/g) || [])).toHaveLength(11); expect(route).not.toMatch(/\/execute|\/apply|\/probe/); expect(route).toContain("create('/flow-log-batches'");
    expect(api).toContain('getNetworkAdvanced()'); expect(api).toContain('ingestNetworkFlowLogs(body)'); expect(ui).toContain("_tabButton('network-advanced'"); expect(ui).toContain('Advanced network execution boundary'); expect(ui).toContain('no raw payload');
  });
});
