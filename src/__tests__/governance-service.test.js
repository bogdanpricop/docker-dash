...
describe('GovernanceService (V4.6a)', () => {
  ...
  test('seeds the catalog and lets an inherited binding grant only its declared permission', () => {
    ...
    expect(catalog.permissions).toHaveLength(123);
    expect(catalog.permissions).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ permission_key: 'compose_blueprint.instantiate', key: 'compose_blueprint.instantiate' }),
      expect.objectContaining({ permission_key: 'self_service.fulfill', key: 'self_service.fulfill' }),
      ...
      expect.objectContaining({ permission_key: 'infrastructure_gitops.manage', key: 'infrastructure_gitops.manage' }),
      expect.objectContaining({ permission_key: 'infrastructure_webhook.manage', key: 'infrastructure_webhook.manage' }),
      expect.objectContaining({ permission_key: 'infrastructure_operations.manage', key: 'infrastructure_operations.manage' }),
      expect.objectContaining({ permission_key: 'lifecycle_updates.manage', key: 'lifecycle_updates.manage' }),
      expect.objectContaining({ permission_key: 'finops_sustainability.manage', key: 'finops_sustainability.manage' }),
      expect.objectContaining({ permission_key: 'finops_tco.manage', key: 'finops_tco.manage' }),
      expect.objectContaining({ permission_key: 'kubernetes_virtualization.manage', key: 'kubernetes_virtualization.manage' }),
      expect.objectContaining({ permission_key: 'kubernetes_unified.manage', key: 'kubernetes_unified.manage' }),
      ...
      expect.objectContaining({ permission_key: 'portal.branding.manage', key: 'portal.branding.manage' }),
      expect.objectContaining({ permission_key: 'portal.support.read', key: 'portal.support.read' }),
      expect.objectContaining({ permission_key: 'portal.incident.manage', key: 'portal.incident.manage' }),
      expect.objectContaining({ permission_key: 'feedback.manage', key: 'feedback.manage' }),
      expect.objectContaining({ permission_key: 'hardware_inventory.read', key: 'hardware_inventory.read' }),
      expect.objectContaining({ permission_key: 'hardware_performance.manage', key: 'hardware_performance.manage' }),
      ...
      expect.objectContaining({ permission_key: 'migration_factory.manage', key: 'migration_factory.manage' }),
      expect.objectContaining({ permission_key: 'migration_cutover.manage', key: 'migration_cutover.manage' }),
      expect.objectContaining({ permission_key: 'platform_inventory.manage', key: 'platform_inventory.manage' }),
      expect.objectContaining({ permission_key: 'platform_metadata.manage', key: 'platform_metadata.manage' }),
      expect.objectContaining({ permission_key: 'platform_content.manage', key: 'platform_content.manage' }),
      expect.objectContaining({ permission_key: 'content_lifecycle.manage', key: 'content_lifecycle.manage' }),
      expect.objectContaining({ permission_key: 'vm_guest_commands.manage', key: 'vm_guest_commands.manage' }),
      expect.objectContaining({ permission_key: 'migration_operations.manage', key: 'migration_operations.manage' }),
      expect.objectContaining({ permission_key: 'privileged.elevation.request', key: 'privileged.elevation.request' }),
      expect.objectContaining({ permission_key: 'compliance.evidence.export', key: 'compliance.evidence.export' }),
    ]));
