...
const marker = 'dd-ha-lease-' + crypto.randomBytes(6).toString('hex'), owned = [];
...
const sources = ['src/services/cluster.js', 'src/services/cluster-lease.js',
  'scripts/fixtures/ha-lease-node.js', 'scripts/fixtures/ha-lease-checks.js'];
...
(async () => {
  try {
    ...
    const controller = await docker.createContainer({ name: marker + '-check', Image: image,
      Entrypoint: ['node'], Cmd: ['scripts/fixtures/ha-lease-checks.js'], Labels: labels, WorkingDir: '/app',
    ...
    assert.equal(info.State.Running, false, 'HA controller timeout'); assert.equal(info.State.OOMKilled, false);
    ...
    console.log('Removed own HA canary containers and ephemeral Redis data');
  }
